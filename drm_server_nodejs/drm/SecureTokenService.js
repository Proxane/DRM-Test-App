	...
	module.exports = {
		"createRouter": function createRouter() {
			...
			router.get("/:videoName",
				async function processGet(request, response) {
				...
				//If video is premium only, check if  user is authorized to watch
				

				if (video.license === "premium") {
					if (!request.header('Authorization')){
						response.status(401).send("Unauthorized to watch this video")
						return;
					}
